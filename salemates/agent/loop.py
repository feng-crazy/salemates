...
from salemates.agent.emotion.analyzer import CustomerEmotion, EmotionAnalyzer, EmotionResult
from salemates.agent.intent.recognizer import IntentRecognizer, IntentResult, SalesIntent
...
from salemates.agent.safety.guardrails import GuardrailManager, create_default_guardrails
from salemates.agent.subagent import SubagentManager
from salemates.agent.tools import register_default_tools
from salemates.agent.tools.registry import ToolRegistry
from salemates.bus.events import InboundMessage, OutboundMessage, OutboundEventType
...
from salemates.hooks import HookContext
from salemates.hooks.manager import hook_manager
from salemates.providers.base import LLMProvider
from salemates.sandbox import SandboxManager
from salemates.session.manager import SessionManager
from salemates.utils.helpers import cal_str_tokens
from salemates.utils.tracing import trace
...
class AgentLoop:
    ...
    def __init__(
        ...
        Example:
            ...
            ... )
        """
        from salemates.config.schema import ExecToolConfig
        ...
        self.intent_recognizer = IntentRecognizer(provider) if enable_intent else None
        self.emotion_analyzer = EmotionAnalyzer(provider, model=model) if enable_emotion else None
        self.emotion_fuse = EmotionFuse(EmotionFuseConfig()) if enable_emotion else None
        self.guardrail_manager = create_default_guardrails() if enable_guardrails else None
    ...
    async def _process_message(self, msg: InboundMessage) -> OutboundMessage | None:
        ...
        # Analyze intent and emotion for sales context
        intent_result = None
        emotion_result = None

        if self.intent_recognizer:
            try:
                intent_result = await self.intent_recognizer.recognize(msg.content)
                session.metadata["last_intent"] = intent_result.to_dict()
                logger.info(
        ...
        if self.emotion_analyzer:
            try:
                emotion_result = await self.emotion_analyzer.analyze(msg.content)
                session.metadata["last_emotion"] = emotion_result.to_dict()
                logger.info(
                    f"Emotion: {emotion_result.emotion.value} (intensity: {emotion_result.intensity:.2f})"
                )
            except Exception as e:
                logger.warning(f"Emotion analysis failed: {e}")

        # Check emotion fuse for human handoff
        if self.emotion_fuse and emotion_result:
            fuse_result = self.emotion_fuse.check(emotion_result, msg.content)
            session.metadata["fuse_action"] = fuse_result.action.value

            if fuse_result.action.requires_human:
                logger.warning(
                    f"Emotion fuse triggered: {fuse_result.action.value} - {fuse_result.reason}"
                )
                return OutboundMessage(
                    session_key=msg.session_key,
                    content="我正在为您转接专属客服，请稍候...",
                    metadata={"fuse_triggered": True, "fuse_reason": fuse_result.reason},
                )
